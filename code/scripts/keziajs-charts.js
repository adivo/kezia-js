...
define(["class_require-mod", "common", "tags", "keziajs"], function(OO, Common, Tags, K) {
         ...
         * The spread method is only relevant if the gradient does not fill out the shape completely (see offset attributes of the <stop> elements).
         * @returns {String}
         */
        ...
        var Coord = OO.Class.extend(new function() {
                 ...
                 * @returns returns
                 */
                this.renderLegend = function(series, seriesColors, posX, posY, width, height, legendFormat, legendBgColor) {
                    var code = '';
                    var seriesCount = Object.keys(series).length;
                    code += this.rect(posX, posY, width, height, 5, 5, '', 'style="stroke:#e0e0e0; fill: ' + legendBgColor + '"', '');
                    ...
                    if (legendFormat === m.LegendFormat.CENTERED {
                            ...
                            for (var i = 0; i < seriesCount; i++) {
                                var seriesColor = this.rect(x, y, 10, 10, 2, 2, '',
                                    'stroke-width="0" fill="' + seriesColors[i] + '" ', '');
                                var seriesName = this.text(x + 15, y - 10, '', series[i]);
                                code += seriesColor + seriesName;
                                x += widthOneItem;
            ...
            this.rectToAnimateId = []; this.rectToAnimateHeight = []; this.rectToAnimateY;
        };
        ...
        this.renderChart = function(componentWidth, componentHeight) {
            ...
            m.SuperChart = K.Component.extend(new function() {
                ...
                this.renderChart = function(componentWidth, componentHeight) {
                    ...
                    chart+= cc.renderLegend(series, seriesColors, this.legendX, this.legendY, this.legendWidth, this.legendHeight, m.LegendFormat.LEFT_ALIGNED,'#404040');
                  /*  var seriesCount = Object.keys(series).length;
