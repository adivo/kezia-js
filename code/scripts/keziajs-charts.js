...
//Object to be exported. Define any public object and functions on this object.  
    ...
    m.ChartModel = OO.Class.extend(new function () {
        this.init = function (dataArray, dimensionInd, seriesInd, factInd) {
            ...
            for (dataKey in dataArray) {
                ...
                var series = data[seriesInd];
                seriesNameCollection[series] = series;
                var modelItem = Common.valueOrDefault(this.model[dimension], {});
                var fact = Number(Common.valueOrDefault(modelItem[series], 0)) + Number(data[factInd]);
                if (fact > this.maxValue) {
                    this.maxValue = fact;
                }
                modelItem[series] = fact;
                this.model[dimension] = modelItem;
            }

            //add missing series
            for (var key in this.model) {
                var modelItem = this.model[key];
                for (var serie in seriesNameCollection) {
                    if (Common.isUndef(modelItem[serie])) {
                        modelItem[serie] = 0;
                    }
...
//          
        ...
        this.renderChart = function (componentWidth, componentHeight) {
            var modelObj = new m.ChartModel(this.model, 0, 1, 2);
