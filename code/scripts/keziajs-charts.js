...
//            var aggregateToDisplay = aggregates[drillDownPath];
            this.topPadding = Common.valueOrDefault(this.topPadding, 40);
            this.bottomPadding = Common.valueOrDefault(this.bottomPadding, 100);
            this.leftPadding = Common.valueOrDefault(this.leftPadding, 100);
            this.rightPadding = Common.valueOrDefault(this.rightPadding, 50);
            this.legendWidth = Common.valueOrDefault(this.legendWidth, 120);
            this.legendHeight = Common.valueOrDefault(this.legendHeight, 15 * seriesCount);
            this.legendX = Common.valueOrDefault(this.legendX, componentWidth - 5 - this.legendWidth);
            this.legendY = Common.valueOrDefault(this.legendY, componentHeight - 5 - this.legendHeight);

            var areaWidth = componentWidth - this.leftPadding - this.rightPadding;
            var areaHeight = componentHeight - this.topPadding - this.bottomPadding;
            var slotWidth = Math.round(areaWidth / (itemCount * (series.length + 1)));
            var colWidth = Math.round(slotWidth * 0.75);
            var c = new Coord(this.leftPadding, componentHeight - this.bottomPadding);
            var chart = '<svg width="100%" height="100%">';
            chart += '<defs>'
            ...
            chart += '<rect x="' + c.x(0) + '" y="' + c.y(areaHeight) + '" width="' + (areaWidth) + '" height="' + (areaHeight) + '" rx="02" ry="2"'
                    ...
                    + ' stroke: #a0a0a0;'
                    + ' stroke-width: 0;" />';
            chart += c.text(areaWidth / 2, areaHeight + 30, 'style="text-anchor: middle"', this.title);
            chart += c.text(areaWidth / 2, areaHeight + 10, 'style="font-size: 10px;text-anchor: middle"', this.subtitle);
            //calculate rendering scale
            var scale = (areaHeight - 60) / maxValue;
            // render y-Axis with values
...
//                var timeRect = end - start;
            ...
            var x = areaWidth;
            var seriesCount = Object.keys(series).length;
            chart += c.rect(this.legendX, areaHeight - 5, 90, seriesCount * 20, 5, 5, '', 'style="stroke:#e0e0e0; fill: #f0f0f0"', '');
